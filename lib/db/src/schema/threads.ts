...
export const threads = pgTable("threads", {
  ...
  // Deprecated: no longer written to. Historical rows were nulled out by a
  // one-time migration (2026-07-10) — display handle is now ALWAYS computed
  // at read time from `createdBy` + `isAutoGenerated` via anonHandle(), never
  // trusted from this column, so anonymization can't drift out of sync with
  // the leaderboard convention. Kept only to avoid a breaking schema removal.
